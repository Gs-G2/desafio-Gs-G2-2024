...
describe('Recintos do Zoologico', () => {
    ...
    test('Deve rejeitar input de tipo errado para animal', () => {
    ...
    test('Deve rejeitar input de tipo errado para quantidade', () => {

        const resultado = new RecintosZoo().analisaRecintos("MACACO", "MACACO");
        expect(resultado.erro).toBe("Quantidade inválida");
        expect(resultado.recintosViaveis).toBeFalsy();
    });

    test('Deve rejeitar inputs numericos invalidos adicionais', () => {
        const resultado = new RecintosZoo().analisaRecintos('MACACO', -1);
        expect(resultado.erro).toBe("Quantidade inválida");
        expect(resultado.recintosViaveis).toBeFalsy();
    });
